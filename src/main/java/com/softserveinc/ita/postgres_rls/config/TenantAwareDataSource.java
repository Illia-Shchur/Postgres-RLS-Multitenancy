...
public class TenantAwareDataSource extends DelegatingDataSource {
    ...
    private void setTenantId(Connection connection) throws SQLException {
        List<Long> currentTenants = TenantContext.getCurrentTenants();
        if(currentTenants == null){
            return;
        }
        try (Statement sql = connection.createStatement()) {
            String tenantId = toSqlArray(currentTenants);
            sql.execute("SET app.user_tenants TO '" + tenantId + "'");
        }
    }

    private String toSqlArray(List<?> list) {
